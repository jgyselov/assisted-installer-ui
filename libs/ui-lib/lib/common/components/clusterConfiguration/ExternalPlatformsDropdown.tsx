...
const ExternalPlatformsDropdown = ({ isDisabled }: { isDisabled: boolean }) => {
  ...
  return isDisabled ? (
    ...
    <FormGroup
      isInline
      label={t('ai:Integrate with partner external platforms')}
      isRequired
      name={'platform'}
    >
      <Dropdown
        ...
        toggle={(toggleRef) => (
          <MenuToggle ref={toggleRef} className="pf-v5-u-w-100" onClick={() => setIsOpen(!isOpen)}>
            {value ? platforms[value] : t('ai:Integrate with external platforms')}
          </MenuToggle>
        )}
        onSelect={onSelect}
      >
...
export default ExternalPlatformsDropdown;
